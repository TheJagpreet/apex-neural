...
The extension contributes a single Language Model Tool called `apex-neural_memory` (referenced as `#apex_memory` in chat) with three actions:
...
```
#apex_memory store a new memory for the architect agent about the API design patterns we discovered
```
...
```
#apex_memory recall memories about API validation
```
...
```
#apex_memory list all memories for the tester agent
```
